...
# CORREZIONE IMPORT: Nelle nuove versioni si importa direttamente così
...
def fetch_users():
    try:
        ...
        # Struttura richiesta da streamlit-authenticator v0.3.x
        usernames = {u["username"]: {
            "name": u["name"], 
            "password": u["password"], 
            "email": u.get("email", "") # Aggiunto campo email per compatibilità
        } for u in res.data}
...
# --- 3. AUTENTICAZIONE ---
# Nota: La versione 0.3.1 richiede questa precisa struttura
...
if not st.session_state.get("authentication_status"):
        ...
        with st.form("signup_form"):
            ...
            if st.form_submit_button("Crea Account"):
                if p != cp:
                    ...
                    # Hash della password
                    hashed_pw = Hasher([p]).generate()[0]
                    sb.table("profiles").insert({
                        "username": u, "name": n, "password": hashed_pw, "is_premium": False
                    }).execute()
    ...
    res_premium = sb.table("profiles").select("is_premium").eq("username", curr_user).single().execute()
    is_premium = res_premium.data.get("is_premium", False) if res_premium.data else False
    ...
    t1, t2, t3 = st.tabs(["📝 Diario", "📈 Analisi & PDF", "🔬 Coach IA"])
    ...
    with t2:
        if not is_premium:
            ...
            res_data = sb.table("wellness").select("*").eq("user_id", curr_user).order("created_at").execute()
            if res_data.data:
                ...
                fig.add_trace(go.Bar(x=df['created_at'], y=df['ore_sonno'], name="Ore Sonno"))
                ...
                if st.button("📄 Genera Report PDF"):
                    ...
                    for _, row in df.tail(15).iterrows():
                        ...
                        pdf.cell(60, 10, str(row['fatica']), 1); pdf.cell(60, 10, str(row['ore_sonno']), 1, 1)
                    
                    pdf_out = pdf.output(dest='S').encode('latin-1', errors='replace')
                    st.download_button("⬇️ Scarica PDF", pdf_out, "report.pdf", "application/pdf")
            else:
    ...
    with t3:
        ...
        if c2.button("🥗 Dieta Turnista"): fast_q = "Cosa mangiare durante il turno notturno per restare vigili?"
        if c3.button("☕ Stop Caffeina"): fast_q = "Qual è l'orario migliore per l'ultimo caffè prima di dormire?"
